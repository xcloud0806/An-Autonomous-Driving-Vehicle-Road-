...
def get_coll_annotation(
    ...
    if obs_data_path == "" or obs_res_path == "" or \
        not os.path.exists(obs_data_path) or not os.path.exists(obs_res_path):
        print("skip {} obstacle anno data submit in {}".format(seg_meta['seg_uid'], coll_id))
    else:
        pack_obstacle = True
        annotation[ANNO_INFO_OBSTACLE_KEY] = gen_label_obstacle(
            segid, obs_res_path, obs_data_path, seg_meta
        )
...
def node_main(run_config: dict):
    ...
    if 'clip_obstacle' in pre_anno_cfg:
        clip_obstacle = pre_anno_cfg["clip_obstacle"]
        clip_obstacle = os.path.dirname(clip_obstacle)
    else:
        clip_obstacle = None

    if 'clip_obstacle_test' in pre_anno_cfg:
        clip_obstacle_test = pre_anno_cfg["clip_obstacle_test"]
        clip_obstacle_test = os.path.dirname(clip_obstacle_test)
    else:
    ...
    coll_root = multi_cfg["origin_coll_path"]
    ...
    for coll in collects:
        ...
        coll_id = coll
        segs = coll_info[coll]["clips_path"]
        ...
        for seg_path in segs:            
            ...
            lane_anno_path = None
            lane_path = os.path.join(clip_lane, coll_id)
            if clip_lane_anno_path is not None:
                lane_anno_path = os.path.join(clip_lane_anno_path, coll_id)
            if clip_obstacle is not None and clip_obstacle_test is not None:
                obstacle_path = os.path.join(clip_obstacle, seg_subfix, segid)
                if not os.path.exists(obstacle_path):
                    obstacle_path = os.path.join(clip_obstacle_test, seg_subfix, segid)
            else:
                obstacle_path = ""
            obstacle_anno_path = ""
